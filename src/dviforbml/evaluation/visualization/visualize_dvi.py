...
def visualize_dds_1d(
    device: torch.device,
    model: DVI,
    dataset: ContextTestDataset,
    num_samples: int = 256,
) -> None:
    assert isinstance(model.cdvi, DDS)
    ...
    _, z_samples = model.cdvi.compute_loss(target_dist, r, None, s)
    z_samples = z_samples.permute(2, 0, 1, 3)
